package com.devsapiens.phonemagic.ui.theme

import androidx.compose.ui.graphics.Color

// Modern, attractive palette
val Navy900 = Color(0xFF061028)
val Navy800 = Color(0xFF0B2340)
val Navy700 = Color(0xFF12304A)
val Coral = Color(0xFFFF6B6B)
val CoralDark = Color(0xFFDD5252)
val Mint = Color(0xFF6EE7B7)
val SoftYellow = Color(0xFFFFE082)

val LightBackground = Color(0xFFF6FAFF)
val LightSurface = Color(0xFFFFFFFF)
val DarkBackground = Color(0xFF071428)
val DarkSurface = Color(0xFF0B1724)

// Semantic aliases used in the app theme
val Primary = Coral
val PrimaryVariant = CoralDark
val Secondary = Mint
val SurfaceColor = LightSurface
val BackgroundColor = LightBackground
